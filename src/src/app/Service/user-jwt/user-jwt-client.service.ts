...
export class UserJwtClientService {
  ...
  getGeneratedToken(requestBody: any) {
    ...
    return this.http.post(this.singleurl + "authenticate", requestBody, { responseType: 'text' as 'json' });

  }
  getRole(firstName:String){
   return this.http.get<string>(this.singleurl + `gettherole/${firstName}`, { responseType: 'text' as 'json' });
  }
